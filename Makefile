...
run:
	...
	export DJANGO_SETTINGS_MODULE='nodraft.settings.staging';\
	poetry run gunicorn nodraft.wsgi:application --bind localhost:8000
...
lint:
	...
	poetry run autopep8 nodraft -r -d
	poetry run autopep8 nodraft -r -i
	@echo '---Running isort---'
	poetry run isort nodraft --diff
	poetry run isort nodraft --atomic
